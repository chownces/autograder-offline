...
function initFramebufferObject() {
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.drawingBufferWidth, gl.drawingBufferHeight, 0, gl.RGBA, gl.UNSIGNED_BYTE, texture);
...
function drawRune(first, indexCount, instanceArray) {
    ...
    instance_ext.drawElementsInstancedANGLE(gl.TRIANGLES, indexCount, gl.UNSIGNED_SHORT, first * indexSize, instanceCount);
...
function initRune3d() {
    ...
    mat4.lookAt(leftCameraMatrix, vec3.fromValues(-halfEyeDistance, 0, 0), vec3.fromValues(0, 0, -0.4), vec3.fromValues(0, 1, 0));
    rightCameraMatrix = mat4.create();
    mat4.lookAt(rightCameraMatrix, vec3.fromValues(halfEyeDistance, 0, 0), vec3.fromValues(0, 0, -0.4), vec3.fromValues(0, 1, 0));
    // set up frame buffers
    if (!((leftFramebuffer = initFramebufferObject()) && (rightFramebuffer = initFramebufferObject()))) {
